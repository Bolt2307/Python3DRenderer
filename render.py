...
def handle_control():
    ...
    if not pause: # When unpaused
        ...
        if keys[pygame.K_e]:
            ...
            objects[find_obj.by_id("cube" + str(cubenum))].position = objects[find_obj.by_id("wire_cube")].position
            objects[find_obj.by_id("cube" + str(cubenum))].visible = True
            cubenum += 1
        if keys[pygame.K_q]:
            try:
                del objects[find_obj.by_position(objects[find_obj.by_id("wire_cube")].position, objects, ["wire_cube"])]
            except:
                pass
...
def update():
    ...
    if not pause:
        ...
        objects[find_obj.by_id("wire_cube")].position = Vector3(x, y, z)
